...
Tube::Tube()
    : mNumSegs(16), mScale0(1), mScale1(1)
{
    makeCircleProfile(mProf);
}

Tube::Tube(const Tube& obj)
    : mNumSegs(obj.mNumSegs), mBSpline(obj.mBSpline), mProf(obj.mProf),
    mScale0(obj.mScale0), mScale1(obj.mScale1), mPs(obj.mPs), mTs(obj.mTs),
    mFrames(obj.mFrames)
{
}
...
void makeCircleProfile( std::vector<Vec3f>& prof, float rad, int segments )
{
    ...
    for( int i = 0; i < segments; ++i ) {
        float t = i*dt;
        prof.push_back( Vec3f( cos( t )*THREAD_HRT, sin( t )*THREAD_VERT, 0 ) );
    }
}
