...
#define THREAD_HRT 0.025f
#define THREAD_VERT 0.05f
...
class Tube
{
    public:
        Tube();
        Tube( const Tube& obj );
        Tube( const ci::BSpline3f& bspline, const std::vector<cinder::Vec3f>& prof );
        ~Tube() {}

        Tube& operator=( const Tube& rhs );

        void					setBSpline( const ci::BSpline3f& bspline ) { mBSpline = bspline; }
        void					setProfile( const std::vector<cinder::Vec3f>& prof ) { mProf = prof; }
    ...
    private:
        ...
        std::vector<cinder::Vec3f>   mProf;			// points to tube (read extrude)
        float					mScale0;		// min scale of profile along curves
        float					mScale1;		// max scale of profile along curves
        std::vector<cinder::Vec3f>   mPs;			// Points in b-spline sample
        std::vector<cinder::Vec3f>   mTs;			// Tangents in b-spline sample
        std::vector<cinder::Matrix44f>	mFrames;		// Coordinate frame at each b-spline sample
};


void makeCircleProfile( std::vector<cinder::Vec3f>& prof, float rad = 0.25f, int segments = 16 );

void makeStarProfile( std::vector<cinder::Vec3f>& prof, float rad = 0.25f );

void makeHypotrochoid( std::vector<cinder::Vec3f>& prof, float rad = 0.25f );

void makeEpicycloid( std::vector<cinder::Vec3f>& prof, float rad = 0.25f );
